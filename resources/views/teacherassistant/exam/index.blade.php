...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <div class="product-status-wrap drp-lst">
        ...
        <div class="add-product">
            <a href="/sectionexam/create">@lang('home.addexam')</a>
        </div>
        <div class="asset-inner">
            <table>
                ...
                @foreach(auth()->user()->exams as $k => $exam)
                <tr>
                    <td>{{$k+1}}</td>
                    ...
                    <td>{{$exam->start}}</td>
                    <td>{{$exam->end}}</td>
                    <td>{{$exam->sectiongroup}}</td>
                    <td>
                        <a data-toggle="tooltip" title="" href="/sectionexam/{{$exam->id}}/edit" class="pd-setting-ed" data-original-title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                        <a data-toggle="tooltip" title="" class="pd-setting-ed" data-original-title="Trash" onclick="event.preventDefault();document.getElementById('delete').submit();"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                           <form id="delete" action="{{ route('sectionexam.destroy',['id'=>$exam->id]) }}" method="POST" style="display: none;">
                               @csrf
                               @method('DELETE')
                           </form>
                    </td>
